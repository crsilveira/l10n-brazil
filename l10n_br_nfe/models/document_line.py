            ...
            if self.ibs_base:
                ...
                if self.ibs_percent is not None or self.ibs_value:
                    ...
                    if self.ibs_reduction:
                        gred_parts = []
                        aliqEfet = self.ibs_percent * (1 - (self.ibs_reduction / 100))
                        gred_parts.append(
                            f"<pRedAliq>{self.ibs_reduction:.4f}</pRedAliq>"
                            f"<pAliqEfet>{aliqEfet:.4f}</pAliqEfet>"
                        )
                        gibsuf_parts.append(f"<gRed>{''.join(gred_parts)}</gRed>")
                ...
                if self.ibs_reduction:
                    ...
                    gibsmun_parts.append(f"<gRed>{''.join(gred_parts)}</gRed>")
                ...
                if self.cbs_reduction:
                    gred_parts = []
                    aliqEfet = self.cbs_percent * (1 - (self.cbs_reduction / 100))
                    gred_parts.append(
                        f"<pRedAliq>{self.cbs_reduction:.4f}</pRedAliq>"
                        f"<pAliqEfet>{aliqEfet:.4f}</pAliqEfet>"
                    )
                    gcbs_parts.append(f"<gRed>{''.join(gred_parts)}</gRed>")
